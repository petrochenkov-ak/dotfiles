...
# Изменено: убрали 'views' из структуры путей, теперь генерируем роутеры
TARGET_DIR = Path("python/packages/gen_django_routers/src/gen_django_routers/routers")
ROOT_DIR = Path(".")
...
def convert_url_to_ninja(url):
    """Convert URL path parameters from {param} format into Django Ninja format: {param:type}."""
    clean_url = url.strip("/")
    def replace_param(match):
        param = match.group(1)
        param_type = "int" if "id" in param.lower() else "str"
        return f"{{{param}:{param_type}}}"
    ninja_url = re.sub(r'\{([^}]+)\}', replace_param, clean_url)
    return f"/{ninja_url}"
...
def main():
    ...
    # Validation 2: Ensure at least one route was processed/generated successfully overall
    ...
    # Создание пустого __init__.py в корне проекта
    init_file = ROOT_DIR / "__init__.py"
    if not init_file.exists():
        init_file.touch()
        print(f"NEW: {init_file}")

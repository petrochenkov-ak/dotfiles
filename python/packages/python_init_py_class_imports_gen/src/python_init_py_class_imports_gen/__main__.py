...
def to_pascal_case(snake_str: str) -> str:
    ...
    # Разделяем по подчеркиваниям, капитализируем каждое слово и сливаем обратно
...
def process_directory(target_dir: Path) -> None:
    ...
    # Генерируем строки импорта классов (имя класса = PascalCase от имени файла)
    lines = [
        f"from .{f.stem} import {to_pascal_case(f.stem)}\n"
        for f in py_files
    ]
    ...
    # Форматируем путь для вывода relative_to, если возможно
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate __init__.py file with PascalCase class imports."
    )
    parser.add_argument(
        "dir_path",
        type=str,
